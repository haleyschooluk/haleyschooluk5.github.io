...
function sg_lag_score(boardid, scorenum) {
    try {
        ...
        /*LaggedAPI.Scores.save(boardinfo, function(response) {
            if (response.success) {
                console.log('high score saved: ' + scorenum);
            } else {
...
function sg_lag_achi(achiid) {
    try {
        ...
        /*LaggedAPI.Achievements.save(api_awards, function(response) {
            if (response.success) {
                console.log('achievement saved: ' + achiid);
            } else {
                console.log(response.errormsg);
            }
        });*/
...
function sg_lag_ad(isreward) {
    try {
        if (typeof prerollStart === 'undefined') {
            ...
            /*LaggedAPI.APIAds.show('interstitial', 'dragon-vs-bricks', 'dragon-vs-bricks-game.png', function(response) {
                if (response.success) {
                    console.log('ad done');
                    if (isreward) {
...
function sg_banner_create(siz) {
    try {
        ...
        bancont.srcdoc = '<script type="text/javascript" src="http://ad.leadboltmobile.net/show_app_ad.js?section_id=191234692"></script>';
